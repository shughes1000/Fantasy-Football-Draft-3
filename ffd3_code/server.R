...
function(input, output) {
  ...
  output$plot1 = renderPlot({
    ...
    a = ggplot(df_plot, aes(rank_adp, prediction, col=pos))+
      geom_point(data=df_real, mapping=aes(y=avg_scoring), alpha=0.15)+
      geom_line(alpha=0.75, size=2)+
      labs(title="Expected PPR Fantasy Points by ADP", 
    ...
    b = ggplot(df_plot, aes(rank_adp, value, col=pos))+ 
      geom_line(alpha=0.75, size=2)+
